    ...
    <div class="container-fluid mt-2" id="kasircontainer">
        ...
        <div class="row">
            <div class="col-sm-8 mt-4">
                <div class="card card-rounded">
                    <div class="card-header bg-primary text-white">
                        <i class="fa fa-cubes"></i> Data Bahan Baku
                    </div>
                    <div class="card-body p-2">
                                ...
                                <div class="dropdown open mb-3">
                                    ...
                                    <div class="dropdown-menu" style="width:100%" aria-labelledby="triggerId">
                                        <?php foreach ($kat as $r) { ?>
                                            <a class="dropdown-item" href="<?= base_url('transfer?id=' . $r->id . '&nm=' . $r->kategori); ?>">
                                                <?= $r->kategori; ?></a>
                                            <div class="dropdown-divider"></div>
                                        <?php } ?>
                                        <a class="dropdown-item" href="<?= base_url('transfer'); ?>">
                                            Semua Kategori</a>
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-6 mb-3">
                                <form method="get" action="">
                                    <div class="input-group">
                                        <input type="text" class="form-control" value="<?= $this->input->get('cari'); ?>" name="cari" id="cari" placeholder="Cari Transfer">
                                        <div class="input-group-append">
                                            <!-- Button trigger modal -->
                                            <button type="submit" class="btn btn-primary btn-md">
                                                <i class="fa fa-search"></i>
                                            </button>
                                            <a href="<?= base_url('transfer'); ?>" class="btn btn-success btn-md btn-block">
                        ...
                        <div class="table-responsive-1 w-100">
                            ...
                            if ($this->input->get('id')) {
                                ...
                                $url   = base_url('bahanbaku/dtbahanbaku?id=' . (int)$this->input->get('id'));
                            } else if ($this->input->get('cari')) {
                                $wr = ' WHERE nama LIKE "%' . $this->input->get('cari') . '%" OR kategori.kategori LIKE "%' . $this->input->get('cari') . '%"';
                                $url   = base_url('bahanbaku/dtbahanbaku?cari=' . $this->input->get('cari'));
                            } else {
                                $wr = '';
                                $url   = base_url('bahanbaku/dtbahanbaku');
                            }
                            ...
                            if ($total == '0') {
                                echo '<br/><h4>" Tidak ada Bahan Baku "</h4><br/>';
                            }
                            ?>
            ...
            <div class="col-sm-4 mt-4">
                <div class="card card-rounded">
                    <div class="card-header bg-primary text-white">
                        <i class="fa fa-shopping-cart"></i> Detail Transfer
                    </div>

                    <form method="post" id="AddKasir">
                        <div class="card-body p-2">
                            <div class="form-group row">
                                <label for="" class="col-sm-4 col-form-label">NO TRANSFER</label>
                                <div class="col-sm-8">
                                    <input type="text" required readonly class="form-control" value="<?= $no_trf; ?>" name="no_trf" id="no_trf" placeholder="No Transfer">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="" class="col-sm-4 col-form-label">Cabang Tujuan</label>
                                <div class="col-sm-8">
                                    <div class="input-group">
                                        <input type="text" class="form-control" name="pelanggan" id="pelanggan" placeholder="Cabang Tujuan">
                                        <div class="input-group-append">
                                            <!-- Button trigger modal -->
                                        ...
                                        <input type="hidden" name="cabang_id" id="cabang_id" value="0">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group mt-3 row">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" required autocomplete="off" class="form-control" name="atas_nama" id="atas_nama" placeholder="Atas Nama">
                                </div>
                            </div>
...
<!-- Button trigger modal -->
<!-- Modal -->
<div class="modal fade" id="modelId1" tabindex="-1" role="dialog" data-toggle="modal" data-backdrop="static" data-keyboard="false">
...
<!-- Modal -->
<div class="modal fade" id="modelId" tabindex="-1" role="dialog" data-toggle="modal" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fa fa-users"></i> Data Cabang</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <div class="table-responsive">
                    <table id="example2" style="font-size:10pt;" class="table table-bordered table-striped table" width="100%">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Kode Cabang</th>
...
<!-- Modal -->
<div class="modal fade" id="cetak-edit" tabindex="-1" role="dialog" data-toggle="modal" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-lg" role="document">
...
<!-- Modal -->
<div class="modal fade" id="modelIdWA" style="margin-top:5pc;" tabindex="1" role="dialog" data-toggle="modal" data-backdrop="static" data-keyboard="false">
            ...
            <form method="get" action="https://api.whatsapp.com/send" target="_blank">
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="number" value="628" name="phone" autocomplete="off" id="phone" class="form-control" placeholder="" aria-describedby="helpId">
                    </div>
                </div>
                <div class="modal-footer">
...
if ((int)$this->input->get('id')) {
    $url = base_url('bahanbaku/data_bahanbaku?id=' . (int)$this->input->get('id'));
} else {
    $url = base_url('bahanbaku/data_bahanbaku');
}
...
<script>
    ...
    $(document).ready(function() {
        ...
        tabel = $('#example2').DataTable({
            ...
            "ajax": {
                "url": "<?= base_url('cabang/data_cabang'); ?>", // URL file untuk proses select datanya
                "type": "POST"
            },
            "deferRender": true,
            "aLengthMenu": [
                ...
                },
                {
                    ...
                    "render": function(data, type, row, meta) {
                        return `<a href="javascript:void(0)" data-id="${row.id}" 
                                    class="btn btn-info btn-sm cek_pilih" title="Pilih Cabang" role="button">
    ...
    $('#example2 tbody').on('click', '.cek_pilih', function(e) {
        ...
        $.ajax({
            url: "<?= base_url('cabang/cek_cabang'); ?>",
            type: "POST",
            data: {
                "id": id
            },
            ...
            success: function(data) {
                $('#cabang_id').val(data[0].id);
                $('#kode_cabang').val(data[0].nama);
                $('#nama_cabang').val(data[0].nama);
                $('#modelId').modal('hide');
            },
