                                ...
                                <form method="get" action="">
                                    <div class="input-group">
                                        ...
                                        <div class="input-group-append">
                                            ...
                                            <a href="<?= base_url('kasirstok'); ?>" class="btn btn-success btn-md btn-block">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <select class="form-control" required name="status" id="status">
                                                <option value="" disabled selected> - Cara Pembayaran -</option>
                                                <option>Cash</option>
                                                <option>QRIS</option>
                                    ...
                                    <!-- <tr>
                                        ...
                                        <td>
                                            <select class="form-control" required name="pesanan" id="pesanan">
                                                <option value="" disabled> - Kategori Order -</option>
...
if ((int)$this->input->get('id')) {
    $url = base_url('menuutama/data_menu?id=' . (int)$this->input->get('id'));
} else {
    $url = base_url('menuutama/data_menu');
}
        ...
        $("#AddKasir").submit(function(e) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result == 'Kurang') {
                    ...
                    } else if (result == 'NoAkses') {
                        Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
                            text: 'Anda tidak diperkenankan menyimpan transaksi ini !',
                        })
                        // alert('Pembayaran Anda Kurang Dari Total Bayar !');
                        $('#prosesTransaksi').attr('disabled', false);
                        $('#prosesTransaksi').addClass('btn-primary').removeClass(
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
                            text: 'Transaksi gagal disimpan ! Periode tanggal ini sudah di closing',
                        })
                        // alert('Pembayaran Anda Kurang Dari Total Bayar !');
                        $('#prosesTransaksi').attr('disabled', false);
                        $('#prosesTransaksi').addClass('btn-primary').removeClass(
                ...
                error: function(xhr, status, error) {
                    ...
                    $("#prosesTransaksi").html('Simpan Transaksi');
                    // $('#AddKasir')[0].reset();
                }
                // error: function(xmlhttprequest, textstatus, message) {
                //     if (textstatus === "timeout") {
                //         alert("request timeout " + textstatus);
                //     } else {
                //         alert("request timeout " + textstatus + " : " + xmlhttprequest);
                //     }
...
<script>
    ...
    $(document).ready(function() {
        $("#AddPelanggan").submit(function(e) {
            e.preventDefault();
            $.ajax({
                url: "{{route('kasirstok.AddPelanggan')}}",
                type: 'POST',
                data: new FormData(this),
                processData: false,
                contentType: false,
...
<script>
    $('.closing').on('click', function(e) {
        ...
        $.ajax({
            url: "<?= base_url('kasirstok/closing'); ?>",
            type: "POST",
            data: {
                "id": id
            },
